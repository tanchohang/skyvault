...
export const upload = async (req, res, next) => {
    ...
    try {
        ...
        if (fileList === undefined || fileList.length === 0) {
            ...
            const uploadedFolder = fileService.saveFiles({ files: [{ user, project, path: uploadPath }] });
            mkdirSync(uploadPath, { recursive: true }); //create folder
            return res.status(200).json('folder created successfully');
        }
        let tempFileList;
        if (filePublic) {
            tempFileList = fileList.map((file) => ({
                path: file.path,
            ...
            tempFileList = fileList.map((file) => ({
                ...
                originalName: file.originalname,
                mimeType: file.mimetype,
                link: [config.base_url, file.path].join('/'),
                project: project,
                user: req.user_id,
            }));
        }
...
export const readAllFiles = async (req, res, next) => {
    try {
...
export const readTrashedFiles = async (req, res, next) => {
    const { pid } = req.params;
    try {
